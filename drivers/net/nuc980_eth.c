...
extern void phy_hw_reset(void);

#if CONFIG_NUC980_PHY_KSZ8863
...
int nuc980_reset_phy(void)
{
	...
	nuc980_eth_mii_write(phy1_addr, MII_ADVERTISE,
	                     ADVERTISE_10HALF |
	                     ADVERTISE_10FULL |
	                     ADVERTISE_100HALF |
	                     ADVERTISE_100FULL);

	nuc980_eth_mii_write(phy2_addr, MII_ADVERTISE,
	                     ADVERTISE_10HALF |
	                     ADVERTISE_10FULL |
	                     ADVERTISE_100HALF |
	                     ADVERTISE_100FULL);

	nuc980_eth_mii_read(phy1_addr, MII_BMCR, &reg);
	nuc980_eth_mii_write(phy1_addr, MII_BMCR, reg | BMCR_ANRESTART | BMCR_ANENABLE);

	nuc980_eth_mii_read(phy2_addr, MII_BMCR, &reg);
	nuc980_eth_mii_write(phy2_addr, MII_BMCR, reg | BMCR_ANRESTART | BMCR_ANENABLE);

	delay = 5000;
	while(--delay > 0) {
		nuc980_eth_mii_read(phy1_addr, MII_BMSR, &reg);
		if((reg & (BMSR_ANEGCOMPLETE | BMSR_LSTATUS)) == (BMSR_ANEGCOMPLETE | BMSR_LSTATUS))
			break;
		
		nuc980_eth_mii_read(phy2_addr, MII_BMSR, &reg);
		if((reg & (BMSR_ANEGCOMPLETE | BMSR_LSTATUS)) == (BMSR_ANEGCOMPLETE | BMSR_LSTATUS))
			break;
	...
	if(delay == 0) {
		...
		nuc980_eth_mii_read(phy1_addr, MII_LPA, &reg);
		printf("phy1 LPA reg:0x%02X\r\n", reg);

		nuc980_eth_mii_read(phy2_addr, MII_LPA, &reg);
		printf("phy2 LPA reg:0x%02X\r\n", reg);
