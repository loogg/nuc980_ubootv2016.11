...
#define UART6_BA	0xB0076000 /* UART6 Control (High-Speed UART) */
#define UART6		((UART_TypeDef *)UART6_BA)
#define REG_PD_PUSEL	0xB00040F0
/*
...
int nuc980_serial_init (void)
{
	__raw_writel(__raw_readl(REG_PCLKEN0) | 0x400000, REG_PCLKEN0);  // UART6 clk on
	__raw_writel(__raw_readl(REG_HCLKEN) | 0x800, REG_HCLKEN);  	// GPIO clk on
	__raw_writel(__raw_readl(REG_PD_PUSEL) | 0x400000, REG_PD_PUSEL);	// PD11 (UART6 Rx) pull up
	__raw_writel((__raw_readl(REG_MFP_GPD_H) & 0xffff00ff) | 0x2200, REG_MFP_GPD_H); // UART6 multi-function

	/* UART6 line configuration for (115200,n,8,1) */
	UART6->LCR |=0x07;
	UART6->BAUD = 0x30000066;	/* 12MHz reference clock input, 115200 */
	UART6->FCR |=0x02;		// Reset UART6 Rx FIFO
...
void nuc980_serial_putc (const char ch)
{
	while ((UART6->FSR & 0x800000)); //waits for TX_FULL bit is clear
	UART6->x.THR = ch;
	if(ch == '\n') {
		while((UART6->FSR & 0x800000)); //waits for TX_FULL bit is clear
		UART6->x.THR = '\r';
...
int nuc980_serial_tstc (void)
{
	return (!((__raw_readl(UART6_BASE + REG_COM_MSR) & RX_FIFO_EMPTY)>>14));
}
